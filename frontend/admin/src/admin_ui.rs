...
pub fn AdminUserInterface() -> Html {
    ...
    let content = match &ctx.auction_state {
        AuctionState::WaitingForAuction => html! {
            <VerticalStack>
                <h1>{"Аукцион еще не начат"}</h1>
                <setup::SetupAuction/>
                <button class="btn btn-success" onclick={start_auction_cb}>{"Начать аукцион"}</button>
            </VerticalStack>
        },
        AuctionState::AuctionOver(report) => html! {
            <VerticalStack>
                ...
                <button class="btn btn-success" onclick={start_auction_anew_cb}>{"Вернуться в начало аукциона"}</button>
            </VerticalStack>
        },
        ...
        AuctionState::WaitingForItem => {
            ...
            html! {
                <HorizontalStack>
                    <VerticalStack>
                        <h1>{"Выбери предмет для продажи"}</h1>
                        <ChooseItemToSell />
                        <button class="btn btn-danger" onclick={conclude_cb}>{"Завершить аукцион"}</button>
                    </VerticalStack>
                    <VerticalStack>
                        <h1>{"Передать деньги вручную"}</h1>
                        {
                            if ctx.admin_state.holding_account_balance == 0 {
                                html!(
                                    <div class="alert alert-success">
                                        {"Временный счет: "}<MoneyDisplay money={0} />
                                    </div>
                                )
                            } else {
                                html!(
                                    <div class="alert alert-warning">
                                        {"Временный счет: "}<MoneyDisplay money={ctx.admin_state.holding_account_balance} />
                                    </div>
                                )
                            }
                        }
